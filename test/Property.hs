...
import Test.HUnit
import Data.Hermes (withHermesEnv)

import NOM.Builds
import NOM.Parser

assertParse :: ByteString -> IO (ByteString, NixEvent)
assertParse input = withHermesEnv \env -> do
  let res = case parse (parser env) input of
        Done x a -> Just (x, a)
        _ -> Nothing
      ...
      test
        [ "Parse Plan" ~: do
            ...
            assertEqual
              ...
              result
            (rest2, result2) <- assertParse rest
            assertEqual
              "result matches"
              ( PlanDownloads
                  134.19
