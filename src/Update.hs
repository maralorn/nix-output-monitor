...
import           Data.Attoparsec.Text.Lazy      ( maybeResult
                                                , parse
                                                )
 ...
 where
  set s = do
    ...
    pure $ s
      ...
      , completedLocalBuilds = Set.union (completedLocalBuilds s)
...
initalState now = BuildState mempty
                             mempty
                             0
                             mempty
                             mempty
...
planDownloads storePath s@BuildState { outstandingDownloads, plannedCopies } =
  s { outstandingDownloads = Set.union storePath outstandingDownloads
...
uploading host storePath s@BuildState { completedUploads } = s
  { completedUploads = Map.insertWith Set.union
                                      host
                                      (Set.singleton storePath)
                                      completedUploads
  }
